...
export const Candidates = (): JSX.Element => {
    ...
    const handleAction = (type: string, candidate: any) => {
        ...
        if (type === 'message') setMessageModal(true);
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#F8FAFC]">
            ...
            {messageModal && selectedCandidate && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-300 px-4">
                    <div className="bg-white rounded-[40px] w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 border border-gray-100">
                        <div className="p-10 text-center space-y-6">
                            <div className="w-20 h-20 bg-blue-50 text-blue-600 rounded-3xl flex items-center justify-center mx-auto shadow-inner shadow-blue-100">
                                <Mail className="w-10 h-10" />
                            </div>
                            <div>
                                <h3 className="text-2xl font-black text-gray-900 tracking-tight">Contact {selectedCandidate.name.split(' ')[0]}</h3>
                                <p className="text-gray-400 font-bold text-xs uppercase tracking-[2px] mt-2">Send interview invitation or follow-up</p>
                            </div>
                            <div className="space-y-4 text-left">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[2px] pl-1">Message Subject</label>
                                    <input type="text" defaultValue={`Interview Invitation - NextHire`} className="w-full px-5 py-3.5 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-blue-50 transition-all" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[2px] pl-1">Message Body</label>
                                    <textarea rows={4} className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-blue-50 transition-all resize-none" placeholder="Write your message here..."></textarea>
                                </div>
                            </div>
                            <div className="flex flex-col gap-3">
                                <button onClick={() => {
                                    alert(`Message sent to ${selectedCandidate.email}`);
                                    setMessageModal(false);
                                }} className="w-full py-5 bg-blue-600 text-white rounded-[20px] font-black uppercase tracking-[3px] text-xs shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all">Send Message Now</button>
                                <button onClick={() => setMessageModal(false)} className="text-gray-400 font-black text-[10px] uppercase tracking-[3px] hover:text-gray-900 transition-colors">Discard Message</button>
                            </div>
                        </div>
                    </div>
                </div>
