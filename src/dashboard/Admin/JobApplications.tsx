...
export const JobApplications = (): JSX.Element => {
    ...
    const filteredApps = appData.filter(app => {
        ...
        return matchesSearch && matchesPosition && matchesStatus && matchesScore && matchesDateFrom && matchesDateTo;
    });
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [searchQuery, selectedPosition, selectedStatus, selectedScoreRange, filterDateFrom, filterDateTo]);
    ...
    const handleExportApplications = (format: "csv" | "excel") => {
        const headers = ["Candidate Name", "Email", "Job Position", "Applied Date", "Skill Score", "Status"];
        const rows = filteredApps.map((app) => [
            escapeCsvValue(app.name),
            escapeCsvValue(app.email),
            escapeCsvValue(app.position),
            escapeCsvValue(app.date),
            escapeCsvValue(app.score),
        ...
        link.href = url;
        link.download = format === "excel" ? "applications-report.xlsx" : "applications-report.csv";
        link.click();
        URL.revokeObjectURL(url);
        setExportModal(false);
    };

    // Minimal valid PDF for dummy CV download
    const MINIMAL_PDF_BASE64 = "";
    const handleDownloadCV = (app: typeof appData[0]) => {
        const bytes = Uint8Array.from(atob(MINIMAL_PDF_BASE64), (c) => c.charCodeAt(0));
        const blob = new Blob([bytes], { type: "application/pdf" });
        const url = URL.createObjectURL(blob);
        const link = document.createElement("a");
        link.href = url;
        link.download = app.resume || "candidate-resume.pdf";
        link.click();
        URL.revokeObjectURL(url);
    };

    const applyAdvancedFilters = () => {
        if (advancedFilterName.trim()) setSearchQuery(advancedFilterName.trim());
    ...
    const getStatusStyle = (status: string) => {
        switch (status) {
            ...
            case "Shortlist": return "bg-indigo-100 text-indigo-700";
                    ...
                    <div className="bg-white rounded-[32px] w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 mx-4 border border-gray-100">
                        ...
                        <div className="p-8 space-y-8 max-h-[65vh] overflow-y-auto custom-scrollbar">
                            <div className="grid grid-cols-2 gap-8">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[2px] flex items-center gap-2">
                                        <Briefcase className="w-3 h-3 text-blue-500" /> Applied Job
                                    </label>
                                    <p className="font-bold text-gray-800 text-sm">{selectedCandidate.position}</p>
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[2px] flex items-center gap-2">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[2px] flex items-center gap-2">Status</label>
                                    <span className={`inline-flex px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${getStatusStyle(selectedCandidate.status)}`}>
                            ...
                            <div className="space-y-3 pt-2 border-t border-gray-100">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-[2px] flex items-center gap-2">
                                    <FileText className="w-3 h-3 text-blue-500" /> Resume preview
                                </label>
                                <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                                    <p className="text-sm font-bold text-gray-700 mb-2">Resume: {selectedCandidate.resume || "candidate-resume.pdf"}</p>
                                    <button
                                        type="button"
                                        onClick={() => handleDownloadCV(selectedCandidate)}
                                        className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-700 transition-all"
                                    >
                                        <Download className="w-4 h-4" />
                                        Download resume
                                    </button>
                            ...
                            <div className="pt-4 border-t border-gray-100">
                                <button
                                    type="button"
                                    onClick={() => handleDownloadCV(selectedCandidate)}
                                    className="w-full py-4 bg-gray-900 text-white rounded-2xl font-bold flex items-center justify-center gap-3 hover:bg-gray-800 transition-all shadow-xl shadow-gray-200 group"
                                >
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 border border-gray-100 mx-4">
                        ...
                        <div className="p-4 space-y-1">
                            <button type="button" onClick={() => updateStatus("Shortlist")} className="w-full flex items-center gap-3 p-4 hover:bg-indigo-50 rounded-2xl transition-all group">
                                <div className="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center group-hover:bg-indigo-100 transition-all">
                                    <CheckCircle className="w-5 h-5" />
                                </div>
                                <div className="text-left">
                                    <p className="font-bold text-gray-900 text-sm">Shortlist</p>
                                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">Add to shortlist</p>
                                </div>
                            </button>
                            <button type="button" onClick={() => updateStatus("Interview")} className="w-full flex items-center gap-3 p-4 hover:bg-amber-50 rounded-2xl transition-all group">
                                ...
                                <div className="text-left">
                                    <p className="font-bold text-gray-900 text-sm">Schedule Interview</p>
                                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">Schedule screening call</p>
                                </div>
                            </button>
                            <button type="button" onClick={() => updateStatus("Rejected")} className="w-full flex items-center gap-3 p-4 hover:bg-rose-50 rounded-2xl transition-all group">
                                ...
                                <div className="text-left">
                                    <p className="font-bold text-gray-900 text-sm">Reject</p>
                        ...
                        <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            <div className="grid grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="text" placeholder="e.g. LKR 120,000 – LKR 160,000" className="w-full px-5 py-3.5 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-bold placeholder:text-gray-300 outline-none focus:ring-4 focus:ring-blue-50 transition-all" />
                                </div>
                                <div className="space-y-2">
                        ...
                        <div className="p-8 space-y-6">
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <input
                                        ...
                                        className="p-3 bg-gray-50 border border-transparent rounded-xl text-xs font-bold focus:ring-2 focus:ring-blue-100 outline-none"
                                    />
                                    <input
                                        type="date"
                                        value={filterDateTo}
                            ...
                            <div className="space-y-3">
                                ...
                                <select
                                    value={selectedPosition}
                                    onChange={(e) => setSelectedPosition(e.target.value)}
                                    className="w-full p-4 bg-gray-50 border border-transparent rounded-2xl text-sm font-bold focus:ring-2 focus:ring-blue-100 outline-none"
                                >
                                    <option>All Positions</option>
                                    <option>Frontend Developer</option>
                                    <option>Backend Developer</option>
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-[2px] pl-1">Status</label>
                                <select
                                    value={selectedStatus}
                                    onChange={(e) => setSelectedStatus(e.target.value)}
                                    className="w-full p-4 bg-gray-50 border border-transparent rounded-2xl text-sm font-bold focus:ring-2 focus:ring-blue-100 outline-none"
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-[2px] pl-1">Skill Score</label>
                                <select
                                    value={selectedScoreRange}
                                    onChange={(e) => setSelectedScoreRange(e.target.value)}
                                    className="w-full p-4 bg-gray-50 border border-transparent rounded-2xl text-sm font-bold focus:ring-2 focus:ring-blue-100 outline-none"
                            ...
                            <div className="space-y-3">
                                ...
                                <input
                                    ...
                                    className="w-full p-4 bg-gray-50 border border-transparent rounded-2xl text-sm font-bold placeholder:text-gray-300 focus:ring-2 focus:ring-blue-100 outline-none"
                                />
                            </div>
                            <button
                                type="button"
